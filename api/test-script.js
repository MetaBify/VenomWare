...
async function handleTestScript(req, res) {
  ...
  const body = await readTestScriptBody();

  if (!body) {
    res.status(500).send("-- test script body is not configured");
    return;
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    await handleTestScript(req, res);
  } catch (error) {
